...
public class CuiSettingsDialog {
    ...
    public static void buildCategory(){
        ...
        ui.settings.addCategory("@settings.cui.settings", Icon.logic, table -> {
            ...
            table.button("@setting.cui-hud-settings.name", Icon.downOpen, Styles.togglet, () -> hudShown[0] = !hudShown[0]).marginLeft(14f).growX().height(commonHeight).checked(a -> hudShown[0]).padTop(5f).row();
            table.collapser( t -> {
            ...
            table.button("@setting.cui-world-settings.name", Icon.downOpen, Styles.togglet, () -> worldShown[0] = !worldShown[0]).marginLeft(14f).growX().height(commonHeight).checked(a -> worldShown[0]).padTop(5f).row();
            table.collapser( t -> {
                SettingsMenuDialog.SettingsTable subTable = new SettingsMenuDialog.SettingsTable();
            ...
            table.button("@setting.cui-hud-settings-advance.name", Icon.downOpen, Styles.togglet, () -> advanceHudShown[0] = !advanceHudShown[0]).marginLeft(14f).growX().height(commonHeight).checked(a -> advanceHudShown[0]).padTop(5f).row();
            table.collapser( t -> {
                SettingsMenuDialog.SettingsTable subTable = new SettingsMenuDialog.SettingsTable();
